...
Supported: Python, JavaScript/TypeScript/TSX, Go, Rust, C/C++, VB.NET, Ruby, Lua/Luau, shell scripts, Zig, Julia, ReScript, Nix, Verilog/SystemVerilog, SQL, Terraform/OpenTofu structure (`.tf`; generic `.hcl` files are recognized as file nodes), Ansible playbooks/roles/tasks, Vue/Svelte SFCs, Astro files parsed through the TypeScript parser, and Jupyter/Databricks notebooks. Generic YAML is not treated as source code.
