...
The parser currently covers Python, JavaScript, TypeScript/TSX, Go, Rust, C/C++, VB.NET, Lua/Luau, shell scripts, Zig, Julia, ReScript, Nix, Verilog/SystemVerilog, SQL, Vue/Svelte single-file components, Astro files parsed through the TypeScript parser, and Jupyter/Databricks notebooks (`.ipynb`).

Extension-less scripts are detected by shebang for common bash/sh/zsh/ksh/dash/ash, Python, Node, Lua interpreters.
