...
- **本地 embeddings**（`uv sync --extra embeddings`；本 fork 不发布到 PyPI，勿用 `pip install "code-review-graph[embeddings]"`）首次使用时从 HuggingFace 下载 sentence-transformers 模型。你的代码不会离开本机。
